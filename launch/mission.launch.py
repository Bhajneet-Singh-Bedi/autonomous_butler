...
def generate_launch_description() -> LaunchDescription:
    ...
    sim_dir = get_package_share_directory('autonomous_butler')
    ...
    map_yaml_file = os.path.join(sim_dir, 'maps', 'restaurant_map_updated.yaml')
    # map_yaml_file = os.path.join(nav2_bringup_dir, 'maps', 'depot.yaml')
    print("Map Path:- ", map_yaml_file)
    ...
    disp_bringup_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(sim_dir, 'launch', 'display.launch.py')
        ),
    )
    ...
    ld.add_action(disp_bringup_cmd)
